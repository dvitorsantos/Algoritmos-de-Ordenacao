...
public class SelectionSortTest extends Tester implements Runnable {
    private boolean ascending;

    private Timer timer;

    public SelectionSortTest(Element[] small_integer, Element[] medium_integer, Element[] big_integer,
    ...
    public void test(Element[] vector) {
        this.timer.start();
        SelectionSort.sort(vector, this.isAscending());
        this.timer.stop();

        System.out.println("(" + vector[0].getKey().getClass().getSimpleName() + ") SelectionSort with " + vector.length + " elements in " + this.timer.getElapsedTime() + "ms.");
    }
}
